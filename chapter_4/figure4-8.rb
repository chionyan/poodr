...
class Customer
  ...
  # 参加者が希望する旅行の一覧を取得
  # @return [Array<Trip>] 希望する旅行一覧
  #
  def suitable_trips
    TripFinder.new.suitable_trips(@date, @difficulty, @need_bike)
  end
end
...
class TripFinder
  ...
  #
  def suitable_trips(on_date, of_difficulty, _need_bike)
    ret = []
    trips = Trip.suitable_trips(on_date, of_difficulty)
    trips.each do |trip|
      ret << trip if bicycle(trip)
    end
    ret
  end
